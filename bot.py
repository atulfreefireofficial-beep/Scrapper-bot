...
START_VIDEO = "https://files.catbox.moe/fevwxk.mp4"  # Change this to your video URL
...
def get_main_menu(is_admin=False):
    buttons = [
        ...
        [InlineKeyboardButton("👥 𝙎𝙘𝙧𝙖𝙥𝙚𝙧 𝙂𝙧𝙤𝙪𝙥", url="https://t.me/your_scraper_group")],  # Change this
...
async def scr_cmd(client, message):
    ...
    if len(args) < 2 or len(args) > 3:
        await message.reply_text(
            "⚠️ 𝙄𝙣𝙫𝙖𝙡𝙞𝙙 𝙐𝙨𝙖𝙜𝙚!\n\n"
            "𝙐𝙨𝙖𝙜𝙚: `/scr [𝙘𝙝𝙖𝙣𝙣𝙚𝙡_𝙪𝙨𝙚𝙧𝙣𝙖𝙢𝙚] [𝙖𝙢𝙤𝙪𝙣𝙩] [𝙤𝙥𝙩𝙞𝙤𝙣𝙖𝙡: 𝙨𝙩𝙖𝙧𝙩𝙞𝙣𝙜_𝙣𝙪𝙢𝙗𝙚𝙧]`\n\n"
            "𝙀𝙭𝙖𝙢𝙥𝙡𝙚𝙨:\n"
            "`/scr @𝙘𝙝𝙖𝙣𝙣𝙚𝙡_𝙣𝙖𝙢𝙚 100`\n"
            "`/scr @𝙘𝙝𝙖𝙣𝙣𝙚𝙡_𝙣𝙖𝙢𝙚 50 4` → 𝘾𝙖𝙧𝙙𝙨 𝙨𝙩𝙖𝙧𝙩𝙞𝙣𝙜 𝙬𝙞𝙩𝙝 4",
    ...
    try:
        ...
        await message.reply_text("❌ 𝙄𝙣𝙫𝙖𝙡𝙞𝙙 𝙘𝙝𝙖𝙣𝙣𝙚𝙡 𝙪𝙨𝙚𝙧𝙣𝙖𝙢𝙚!")
        return
    
    # Progress message
    progress_msg = await message.reply_text(
        "🔄 𝙎𝙘𝙧𝙖𝙥𝙞𝙣𝙜 𝙞𝙣 𝙋𝙧𝙤𝙜𝙧𝙚𝙨𝙨...\n\n"
        f"📢 𝘾𝙝𝙖𝙣𝙣𝙚𝙡: `{channel_name}`\n"
        f"🔢 𝙇𝙞𝙢𝙞𝙩: `{limit}`\n"
        "⏳ 𝙋𝙡𝙚𝙖𝙨𝙚 𝙬𝙖𝙞𝙩..."
    )
    ...
    if unique_messages:
        file_name = f"scraped_{len(unique_messages)}_{channel_name.replace(' ', '_')}.txt"
        ...
        with open(file_name, 'rb') as f:
            caption = (
                ...
                f"📢 𝙎𝙤𝙪𝙧𝙘𝙚: `{channel_name}`\n"
                f"💳 𝘼𝙢𝙤𝙪𝙣𝙩: `{len(unique_messages)}`\n"
                f"🔄 𝘿𝙪𝙥𝙡𝙞𝙘𝙖𝙩𝙚𝙨 𝙍𝙚𝙢𝙤𝙫𝙚𝙙: `{duplicates_removed}`\n"
                f"👤 𝙍𝙚𝙦𝙪𝙚𝙨𝙩𝙚𝙙 𝙗𝙮: {message.from_user.mention}"
            )
            await progress_msg.delete()
            await client.send_document(message.chat.id, f, caption=caption, reply_markup=get_main_menu(message.from_user.id in ADMIN_IDS))
        os.remove(file_name)
    else:
        await progress_msg.delete()
        await client.send_message(message.chat.id, "❌ 𝙉𝙤 𝘾𝙧𝙚𝙙𝙞𝙩 𝘾𝙖𝙧𝙙𝙨 𝙁𝙤𝙪𝙣𝙙!", reply_markup=get_main_menu(message.from_user.id in ADMIN_IDS))
...
async def start_cmd(client, message):
    ...
    # Send video first, then text with buttons
    try:
        ...
        # If video fails, send only text with buttons
...
async def handle_callback(client, callback_query: CallbackQuery):
    ...
    if data == "show_commands":
        commands_text = (
            "📋 𝘼𝙫𝙖𝙞𝙡𝙖𝙗𝙡𝙚 𝘾𝙤𝙢𝙢𝙖𝙣𝙙𝙨\n\n"
            "𝙈𝙖𝙞𝙣 𝘾𝙤𝙢𝙢𝙖𝙣𝙙𝙨:\n"
            "`/start` - 𝙎𝙩𝙖𝙧𝙩 𝙩𝙝𝙚 𝙗𝙤𝙩 & 𝙨𝙝𝙤𝙬 𝙢𝙚𝙣𝙪\n"
            "`/scr` - 𝙎𝙘𝙧𝙖𝙥𝙚 𝙘𝙧𝙚𝙙𝙞𝙩 𝙘𝙖𝙧𝙙𝙨\n\n"
            "𝙎𝙘𝙧𝙖𝙥𝙞𝙣𝙜 𝙁𝙤𝙧𝙢𝙖𝙩:\n"
            "`/scr [𝙘𝙝𝙖𝙣𝙣𝙚𝙡] [𝙖𝙢𝙤𝙪𝙣𝙩]`\n"
            "`/scr [𝙘𝙝𝙖𝙣𝙣𝙚𝙡] [𝙖𝙢𝙤𝙪𝙣𝙩] [𝙥𝙧𝙚𝙛𝙞𝙭]`\n\n"
            "𝙀𝙭𝙖𝙢𝙥𝙡𝙚𝙨:\n"
            "`/scr @𝙚𝙭𝙖𝙢𝙥𝙡𝙚 100`\n"
            "`/scr @𝙚𝙭𝙖𝙢𝙥𝙡𝙚 50 4`\n\n"
            "𝙎𝙪𝙥𝙥𝙤𝙧𝙩: @froxtbackup"
        ...
        start_text = (
            f"🎉 𝙒𝙚𝙡𝙘𝙤𝙢𝙚 𝙗𝙖𝙘𝙠 {user_name}!\n\n"
            f"🤖 𝘾𝙝𝙤𝙤𝙨𝙚 𝙖𝙣 𝙤𝙥𝙩𝙞𝙤𝙣 𝙗𝙚𝙡𝙤𝙬 👇"
        )
...
if __name__ == "__main__":
    ...
    bot.set_bot_commands([
        {"command": "start", "description": "✨ 𝙎𝙩𝙖𝙧𝙩 𝙗𝙤𝙩 & 𝙨𝙝𝙤𝙬 𝙢𝙚𝙣𝙪"},
        {"command": "scr", "description": "🔍 𝙎𝙘𝙧𝙖𝙥𝙚 𝙘𝙧𝙚𝙙𝙞𝙩 𝙘𝙖𝙧𝙙𝙨 𝙛𝙧𝙤𝙢 𝙘𝙝𝙖𝙣𝙣𝙚𝙡"}
    ])
    
    print("🤖 𝘽𝙤𝙩 𝙎𝙩𝙖𝙧𝙩𝙚𝙙 𝙎𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮!")

...
async def handle_scrape_input(client, message):
    ...
    if session_type == "channel":
        ...
        await message.reply_text(
            f"{get_live_emoji('card')} 𝙀𝙣𝙩𝙚𝙧 𝙩𝙝𝙚 𝙣𝙪𝙢𝙗𝙚𝙧 𝙤𝙛 𝙘𝙖𝙧𝙙𝙨 𝙩𝙤 𝙨𝙘𝙧𝙖𝙥𝙚:\n\n"
        ...
        try:
            ...
            # Build progress message parts
            bin_line = f"{get_live_emoji('bin')} 𝘽𝙄𝙉 𝙁𝙞𝙡𝙩𝙚𝙧: `{bin_filter}`\n" if bin_filter else ""
            country_line = f"{get_live_emoji('country')} 𝘾𝙤𝙪𝙣𝙩𝙧𝙮: `{country_filter}`\n" if country_filter else ""
            
            progress_msg = await message.reply_text(
                f"{get_live_emoji('scrape')} 𝙎𝙘𝙧𝙖𝙥𝙞𝙣𝙜 𝙞𝙣 𝙋𝙧𝙤𝙜𝙧𝙚𝙨𝙨...\n\n"
                f"{get_live_emoji('channel')} 𝘾𝙝𝙖𝙣𝙣𝙚𝙡: `{channel_username}`\n"
                f"{get_live_emoji('card')} 𝙇𝙞𝙢𝙞𝙩: `{amount}`\n"
                f"{bin_line}"
                f"{country_line}"
                f"{get_live_emoji('clock')} 𝙋𝙡𝙚𝙖𝙨𝙚 𝙬𝙖𝙞𝙩..."
            )
            ...
            if unique_messages:
                ...
                # Build caption parts
                bin_cap_line = f"{get_live_emoji('bin')} 𝘽𝙄𝙉: `{bin_filter}`\n" if bin_filter else ""
                country_cap_line = f"{get_live_emoji('country')} 𝘾𝙤𝙪𝙣𝙩𝙧𝙮: `{country_filter}`\n" if country_filter else ""
